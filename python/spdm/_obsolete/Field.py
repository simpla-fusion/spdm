...
class Function2D(object):
    ...
    def __init__(self,  *args, dtype=None, order=None, grid: Grid | typing.Mapping | None = None,  **kwargs):
        self._grid = grid if isinstance(grid, Grid) else Grid(grid, *args)

        if value is None:
            self._array = np.zeros(self._grid.shape)
        else:
    ...
    def interpolator(self):
        return self._grid.interpolator(self.__array__())

    def __call__(self, *args, grid=False, **kwargs):
        if len(args) == 0:
            args = self._grid.points
    ...
    def derivative(self, *args, dx=None, dy=None, **kwargs):
        if self.ndim == 1:
            return self.interpolator.derivative(1, **kwargs)(self._grid.axis[0])
        else:
    ...
    def plot(self, axis, *args, linewidths=0.1, **kwargs):
        axis.contour(*self._grid.xy,  self.__array__(),
