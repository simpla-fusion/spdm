...
if __name__ == '__main__':
    ...
    x = np.linspace(0, scipy.constants.pi*2.0, 128, endpoint=False)
    y = np.sin(x)

    f0 = Function(x, y, is_period=True)
    f1 = Function(np.linspace(0, 1.0, 128), y, is_period=True)

    plot_profiles([
        [(f0, r"$f0(x)$"), (f1, r"$f1(x)=f0(2\pi x)$")],
        [(np.cos(x), r"$cos(x)$"),
         (f0.derivative, r"$d f0(x)$"),
         (f1.derivative/(scipy.constants.pi*2.0), r"$d f1(x)/2\pi$")],
        [(-np.cos(x)+np.cos(0), r"-$cos(x)+cos(0)$"), (f0.antiderivative, r"$\int f0$"),
