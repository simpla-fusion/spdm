...
if __name__ == '__main__':
    ...
    with File.create(DATA_PATH/"g063982.04800", mode="r", format="GEQdsk") as fid:
        doc = fid.read()

    with File.create("../output/test.json", mode="w") as oid:
        oid.write(doc.dump())
        oid.close()
    ...
    with File.create("../output/test.h5", mode="r", format="HDF5") as oid:
        logger.debug(oid.read().dump())

    with File.create("../output/test.nc", mode="w", format="NetCDF") as oid:
        oid.write(doc.dump())
        oid.close()

    with File.create("../output/test.nc", mode="r", format="NetCDF") as oid:
        logger.debug(oid.read().dump())
