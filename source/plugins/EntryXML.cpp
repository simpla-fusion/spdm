...
// template <>
// std::string EntryPlugin<pugi::xml_node>::name() const { return ""; }
//----------------------------------------------------------------------------------
// level 0
template <>
...
{
    return Entry::element_t(std::string(m_pimpl_.value()));
}
...
{
    pugi::xml_node n = m_pimpl_.child(name.c_str());
    if (n.empty())
    {
...
// attributes
template <>
bool EntryPlugin<pugi::xml_node>::has_attribute(const std::string& name) const { return !m_pimpl_.attribute(name.c_str()).empty(); }

template <>
Entry::element_t EntryPlugin<pugi::xml_node>::get_attribute_raw(const std::string& name) const
{
    return Entry::element_t{std::string(m_pimpl_.attribute(name.c_str()).value())};
}

template <>
void EntryPlugin<pugi::xml_node>::set_attribute_raw(const std::string& name, const Entry::element_t& value)
{
    m_pimpl_.attribute(name.c_str()).set_value(to_string(value).c_str());
}

template <>
void EntryPlugin<pugi::xml_node>::remove_attribute(const std::string& name) { m_pimpl_.remove_attribute(name.c_str()); }
