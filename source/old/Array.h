...
class ZSFC {
   ...
   public:
    ...
    void Reshape(D d, bool is_slow_first = true) {
        ...
        if (is_slow_first) {
            m_strides_[ndim - 1] = 1;
            for (int i = ndim - 2; i >= 0; --i) { m_strides_[i] = m_strides_[i + 1] * m_count_[i + 1]; }
        } else {
            m_strides_[0] = 1;
            for (int i = 1; i < ndim - 1; ++i) { m_strides_[i] = m_strides_[i - 1] * m_count_[i - 1]; }
        }
    }
    ...
    struct iterator {
       ...
       public:
        ...
        iterator(ZSFC<ndim> const& sfc, size_type pos = 0) : m_sfc_(sfc), m_pos_(pos) {
            m_strides_[ndim - 1] = 1;
            for (int i = ndim - 2; i >= 0; --i) { m_strides_[i] = m_sfc_.m_count_[i + 1] * m_strides_[i + 1]; }
        }
        ~iterator() = default;
...
void foreach_range(TRange const& range, TFun const& fun) {
    for (decltype(auto) v : range) { fun(v); }
}
...
auto reduce_range(TRange const& range, TReduction const& reduction, TFun const& fun) {
    ...
    auto res = fun(*it);
    ++it;
    for (; it != ie; ++it) { res = reduction(res, fun(*it)); }
    return res;
}
}  // namespace utility
template <typename...>
...
struct IndexedGetHelper<T, IDX, std::enable_if_t<!(traits::is_array<traits::remove_cvref_t<T>>::value ||
                                                   traits::is_expression<traits::remove_cvref_t<T>>::value)>> {
...
struct ReduceExpressionHelper<TReduction, TExpr, std::enable_if_t<(traits::rank<TExpr>::value == 0 &&
                                                                   traits::number_of_dimensions<TExpr>::value > 0)>> {
    template <typename E>
...
struct EvaluateExpressionHelper<TLHS, TRHS, std::enable_if_t<traits::is_array<TLHS>::value>> {
    ...
    static int eval(R& lhs, E const& rhs) {
        ...
        for (decltype(auto) idx : r) { get_idx(lhs, idx) = get_idx(rhs, idx); }
        return 1;
    }
};
...
#define DEFINE_NTUPLE_FOREACH_MEMBER_FUNCTION(_GLOBAL_FUN_, _MEMBER_FUN_NAME_)                                         \
    ...
    auto _GLOBAL_FUN_(nTuple<V, N...>& v, Args&&... args)                                                              \
        ...
        auto jb = begin(res);                                                                                          \
        for (; ib != ie; ++ib, ++jb) { *jb = _GLOBAL_FUN_(*ib, std::forward<Args>(args)...); }                         \
        return res;                                                                                                    \
    };                                                                                                                 \
    template <typename V, unsigned int... N, typename I>                                                               \
    auto _GLOBAL_FUN_(nTuple<V, N...>& v, std::initializer_list<I> const& list)                                        \
        ...
        auto ib = begin(v), ie = end(v);                                                                               \
        auto jb = begin(res);                                                                                          \
        for (; ib != ie; ++ib, ++jb) { *jb = _GLOBAL_FUN_(*ib, list); }                                                \
        return res;                                                                                                    \
    };                                                                                                                 \
    template <typename V, unsigned int... N, typename I>                                                               \
    auto _GLOBAL_FUN_(nTuple<V, N...>& v, std::initializer_list<std::initializer_list<I>> const& list)                 \
...
V Fill(ArrayBase<T...>& a, V start, V inc = 0) {
    for (decltype(auto) item : a) { start = Fill(item, start, inc); }
    return start;
}
template <typename... T, typename V>
V Fill(ArrayBase<T...>&& a, V start, V inc = 0) {
