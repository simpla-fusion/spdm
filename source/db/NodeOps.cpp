...
static std::map<std::string, std::function<Node(const Node&, const Node&)>> update_ops_map{

    {"$resize", [](const Node& node, const Node& opt) {
         size_t res = 0;
         std::visit(
             sp::traits::overloaded{
                 [&](const std::variant_alternative_t<Node::tags::Array, Node::value_type>& array_p) { array_p->resize(opt.as<int>()); },
...
Node update_op(const Node& node, const std::string& op, const Node& opt)
{
    Node res;
    auto it = update_ops_map.find(op);
