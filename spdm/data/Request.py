    ...
    def traverse(self,  visitor, prev=[], idx=0):
        ...
        if idx >= len(self._path):
            ...
            if stop is not None:
                ...
                res = []
                count = 0
                while count < Request.MAX_SLICE_LENGTH:
                    try:
                    ...
                    count = count + 1
                    start = start+step
                if count >= Request.MAX_SLICE_LENGTH:
                    logger.warning("List length > {Request.MAX_SLICE_LENGTH}")
        ...
        if idx >= len(self._path):
            ...
            if stop is not None:
                ...
                while count < Request.MAX_SLICE_LENGTH:
                    ...
                    start = start+step
                if count >= Request.MAX_SLICE_LENGTH:
                    logger.warning(f"Index > MAX_SLICE_LENGTH({Request.MAX_SLICE_LENGTH})! {prev+[start]}")
